...
void Logging::LogTextToFileTagVerbose( const char* p_tag, unsigned int p_vLevel, const char* p_format, ... )
{
	va_list args;
	va_start (args, p_format);
	WriteToFile(p_format, args);
	va_end (args);
}
...
void Logging::LogTextToConsoleTagVerbose( const char* p_tag, unsigned int p_vLevel, const char* p_format, ... )
{
	va_list args;
	va_start (args, p_format);
	WriteToConsole(p_format, args);
	va_end (args);
}
...
void Logging::SetExclusiveTags( std::string p_tag, ... )
{

}

void Logging::WriteToFile(std::string p_format, va_list p_args )
{
	// current date/time based on current system
	time_t currentTime = time(0);
...
void Logging::WriteToConsole( std::string p_format, va_list p_args )
{
	// current date/time based on current system
	time_t currentTime = time(0);
	tm gmtm;
