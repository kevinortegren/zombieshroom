	...
	void ConnectingState::Enter(const GameStates::PlayData& p_playData)
	{
		...
		if (p_playData.Host)
		{
			if(!m_networkContext.m_server)
			{
				...
				m_networkContext.m_server = std::shared_ptr<RootForce::Network::Server>(new RootForce::Network::Server(g_engineContext.m_logger, g_world, m_sharedSystems.m_worldSystem.get(), p_playData.ServerInfo));
			...
			m_networkContext.m_server->Initialize(p_playData.ServerInfo, false);
