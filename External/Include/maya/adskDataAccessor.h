...
/*!
	...
	\brief Class handling access to adsk::Data::Structures and
	adsk::Data::Associations located in a file.

	This class is used to read and rewrite a collection of adsk::Data::Structure
	and adsk::Data::Associtions from/to an existing file.
*/
...
public:
	...
	*/
	...
	//! \brief Structures are kept in a set the forbids duplicated names.
...
protected:
	...
	/*!
		\fn bool performRead( std::string& errors );

		Invoked by read() to access the current file.

		The current file's name is obtained with fileName().  Concrete
		implementations must read the structures/associations.  Upon success,
		the structures are read from adsk::Data::Structure::allStructures() and
		kept in this instance.  The associations, on the other hand, must be set
		by the derived class by editing the associations map returned from
		associations().
		
		The caller (the read() method) deals with saving/restoring the content of
		adsk::Data::Structure::allStructures() before/after the call, and clearing
		the existing structures/associations present in the instance before
		calling performRead().
