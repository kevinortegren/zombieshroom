	...
	void RagdollSystem::ProcessEntity( ECS::Entity* p_entity )
	{
		...
		if(animation->m_animClip == AnimationClip::RAGDOLL)
		{
			...
			{		
				...
				m_engineContext->m_physics->BuildRagdoll(*(collision->m_handle), animation->m_bones, renderable->m_model->m_animation->GetScene()->mRootNode, nameToIndex);
				...
				ragdoll->m_initialized = true;
			}
			else
			{
				//physics get them cool bones
			}
