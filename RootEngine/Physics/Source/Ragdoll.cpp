...
namespace Ragdoll
{
	...
	void Ragdoll::Activate( glm::mat4 p_bones[20], glm::mat4 p_transform )
	{
		...
		for(int i = 0; i < BodyPart::TOTAL_BONE_AMUNT ; i++) //hardcoded for 14 bones, needs to change
		{
			...
			glm::mat4 toTrans =  p_transform  * m_lastBoneMatrix[i];	
	...
	{
		//Nu ska vi gˆra allt till boxar fˆr det ‰r coolare
		...
		{
			return new btBoxShape(btVector3(0.2f, 0.1f, 0.2f));
