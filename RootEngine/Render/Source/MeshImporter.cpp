...
namespace Render
{
	...
	void MeshImporter::LoadMesh( const std::string p_fileName )
	{
		...
		Render::g_context.m_logger->LogText(LogTag::RENDER, LogLevel::DEBUG_PRINT, "Starting to load mesh    '%s'", p_fileName.c_str());
		if (aiscene) 
		{
			InitFromScene(aiscene, p_fileName);
			Render::g_context.m_logger->LogText(LogTag::RENDER, LogLevel::DEBUG_PRINT, "Successfully loaded mesh '%s'", p_fileName.c_str());
		}
		else 
		{
			Render::g_context.m_logger->LogText(LogTag::RENDER, LogLevel::FATAL_ERROR, "Error parsing '%s': '%s'", p_fileName.c_str(), importer.GetErrorString());
		}
	}
	...
	void MeshImporter::InitMesh( unsigned int p_index, const aiMesh* p_aiMesh )
	{
		...
		for(unsigned int i = 0 ; i < p_aiMesh->mNumFaces ; i++)
		{
			...
			{
				Render::g_context.m_logger->LogText(LogTag::RENDER, LogLevel::FATAL_ERROR, "Error: Mesh nr %d, face nr %d doesn't contain 3 indices!", p_index, i);
			}
		...
		Mesh mesh;
		mesh.Init(&vertices[0], vertices.size(), &indices[0], indices.size());
		m_entries.push_back(mesh);
	}
