...
namespace RootEngine
{
	namespace Network
	{
		...
		bool Networker::Send( std::string p_str )
		{
			...
			if( p_str.length() > 0 )
			{
				...
				tmp += (char)29;
				tmp.append( p_str );
				numbytessent = m_peer->Send( tmp.data(), tmp.length(), HIGH_PRIORITY, RELIABLE, 0, RakNet::UNASSIGNED_RAKNET_GUID, true);
			}
			return numbytessent == p_str.length();
		}
