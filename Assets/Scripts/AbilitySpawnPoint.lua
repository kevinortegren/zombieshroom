...
function AbilitySpawnPoint.OnLoad()
  ...
  ResourceManager.LoadSound("Feedback/switch1-1.wav", bit32.bor(SoundMode.SOUND_LOOP_OFF, SoundMode.SOUND_3D, SoundMode.SOUND_3D_LINEARSQUAREROLLOFF));
end
  ...
  local type = hitCol:GetType();
   --Logging.Log(LogLevel.DEBUG_PRINT, "Entity collided");
      ...
      if tryPickup:GetTryPickup() then
        ...
        Static.Play3DSound("Feedback/switch1-1.wav", 1.0, self:GetTransformation():GetPos(), 10.0, 100.0);
      end
    end
end
